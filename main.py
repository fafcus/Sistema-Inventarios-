...
def generar_firma_sincronizacion(materiales, documentos, movimientos):
    return (
        tuple((m.get("id"), m.get("codigo"), m.get("material"), m.get("unidad"), m.get("categoria"), m.get("ubicacion"), m.get("observaciones")) for m in materiales),
...
def obtener_documento_actual():
    ...
    for d in obtener_documentos_cache():
        if d.get("nombre") == inventario_seleccionado:
            return d
    return None
...
def obtener_material_seleccionado():
    sel = tabla.selection() if tabla else ()
    if not sel:
